...
LOW_FLOOR_SCORE   = 0.001     # Very low score assigned to filtered-out clauses (0..1 scale)
# ----------------------------------
...
def _ea_send(conn, obj, msg_delim: str) -> None:
    ...
    try:
        ...
        pass
# 置于文件顶部或 _ea_print_in 之前
_EA_MUTE_IN = False
_EA_MUTE_IN_TAGS = set()

def _set_in_log_filter(mute: bool, tags_csv: str):
        global _EA_MUTE_IN, _EA_MUTE_IN_TAGS
        _EA_MUTE_IN = bool(mute)
        _EA_MUTE_IN_TAGS = {t.strip() for t in (tags_csv or '').split(',') if t.strip()}
...
# 导入ProofDataCollector（需要在文件顶部添加sys.path）
try:
    from auto_fof_corpus_builder_local import ProofDataCollector
except ImportError:
    # 如果无法导入，定义一个占位符类
    class ProofDataCollector:
        def __init__(self, *args, **kwargs):
            self.enabled = False
        def on_register_clauses(self, *args, **kwargs): pass
        def on_passive_clauses(self, *args, **kwargs): pass  
        def on_given_clause(self, *args, **kwargs): pass
        def on_simplified_clauses(self, *args, **kwargs): pass
        def on_szs_result_out(self, *args, **kwargs): pass
        def on_proof_out(self, *args, **kwargs): pass
        def save_dataset(self, *args, **kwargs): return None

# ---------------------- Minimal interactive EA server (optional) ----------------------
class _EAState:
    def __init__(self, enable_data_collection: bool = False, output_dir: Optional[Path] = None):
        ...
        self.data_collector: Optional[ProofDataCollector] = None
    ...
    def init_data_collector(self, problem_id: str):
        """为新问题初始化数据收集器"""
        if self.enable_data_collection:
            self.current_problem_id = problem_id
            self.data_collector = ProofDataCollector(problem_id, self.output_dir)
            self.dataset_saved = False
            print(f"[EA] 为问题 {problem_id} 初始化数据收集器")
    
    def save_collected_data(self, problem_file: str = ""):
        """保存收集的数据"""
        if self.enable_data_collection and self.data_collector and not self.dataset_saved:
            try:
                if self.data_collector.dataset:
                    output_file = self.data_collector.save_dataset(problem_file, output_format="jsonl")
                    print(f"[EA] 数据集已保存: {output_file}")
                    self.dataset_saved = True
                    return output_file
                else:
                    print("[EA] 警告: 未生成数据集，可能问题未找到证明")
            except Exception as e:
                print(f"[EA] 保存数据集失败: {e}")
        return None
...
def _ea_score_with_ranker(batch: Dict, ranker_script: str, model: str,
    ...
    if artifacts_dir:
        ...
        env = os.environ.copy()
        env["EA_RUN_DIR"] = os.path.abspath(os.path.join(req_dir, ".."))
        subprocess.run(cmd, check=True, cwd=req_dir, env=env)
...
def _ea_handle_scores_req(state: _EAState, msg: Dict[str, Any], args, log_fp=None) -> Dict[str, Any]:
    ...
    if cache_key in state.scores_cache:
        ...
        if component is not None:   res["component"] = component
        if component_id is not None: res["component_id"] = component_id
        if getattr(args, 'echo_clause_ids', False): res["clause_ids"] = req_ids
        if log_fp is not None:
            ...
            log_fp.flush()
        try: print(f"[EA] cache hit for {len(req_ids)} clauses (component={component}/{component_id}).")
        except Exception: pass
        return res

    # 1) 先构造 batch（包含 AST 和 reasoning tags；后面预筛要用）
    batch = make_batch_for_scores_req(
        all_clauses=state.clauses,
        req_ids=req_ids,
        context_size=args.context_size,
        mapping_scope="batch",
        include_ast=True,          # 需要 tags/AST 做预筛
        component=component,
    )
    ...
    # 3) 轻量打分函数（用于预筛）
    def _cheap_prefilter_score(rec: Dict) -> float:
        s = 0.0
        f = rec.get("features", {}) or {}
        cf = rec.get("canonical_formula", "") or ""
        ...
        if '|' not in cf: s += 0.2
        if '=' in cf:     s += 0.3
        if f.get("horn", False): s += 0.2

        # reasoning tags 加权
        if "eq_of_target_functor" in tags: s += 1.0
        if "first_arg_in_goal" in tags:    s += 0.6
        if "touches_target_functor" in tags: s += 0.4
        for t in tags:
        ...
        # SAT 反馈：更偏向“目前未被模型支持”的（pressure）
        if getattr(args, 'include_sat_eval', False) and getattr(state, 'last_sat_eval', None):
            vals = state.last_sat_eval.get(int(rec['id']), [])
            if vals:
                support = sum(1 for v in vals if v) / max(1, len(vals))
                s += 0.3 * (1.0 - support)

        return s


    # 4) 预筛：按 “比例 + 最小保留 + top-K 上限” 组合规则
    import math
    prefilter_on = getattr(args, 'prefilter_enabled', True) if hasattr(args, 'prefilter_enabled') else PREFILTER_ENABLED
    drop_ratio   = float(getattr(args, 'prefilter_drop', 0.0) or 0.0)
    drop_ratio   = max(0.0, min(drop_ratio, 0.999))  # 避免全丢光
    keep_ratio   = 1.0 - drop_ratio
    top_k        = int(getattr(args, 'prefilter_top_k', 0) or 0)      # 0 表示不启用上限
    min_keep     = int(getattr(args, 'prefilter_min_keep', 0) or 0)   # 0 表示不启用下限
    cands        = list(batch.get('candidate_clauses', []))
    N            = len(cands)

    # 计算目标保留数
    target_keep = int(math.ceil(N * keep_ratio)) if drop_ratio > 0.0 else N
    if min_keep > 0:
        target_keep = max(target_keep, min_keep)
    if top_k > 0:
        target_keep = min(target_keep, top_k)
    target_keep = max(1, min(target_keep, N))  # 夹在 [1, N]

    floor_scores = {}
    batch_for_ranker = batch
    if prefilter_on and N > target_keep:
        # 轻量打分排序（同原来的 _cheap_prefilter_score）
        scored = [(_cheap_prefilter_score(rec), rec) for rec in cands]
        scored.sort(key=lambda x: x[0], reverse=True)
        keep = [rec for _, rec in scored[:target_keep]]
        drop = [rec for _, rec in scored[target_keep:]]

        # 给被丢弃的子句一个很低的保底分
        floor_scores = {int(rec['id']): float(LOW_FLOOR_SCORE) for rec in drop}

        # 仅把保留子句交给 LLM
        batch_for_ranker = dict(batch)
        batch_for_ranker['candidate_clauses'] = keep
        try:
            print(f"[EA] PREFILTER: candidates {N} -> {len(keep)} "
                  f"(drop={drop_ratio:.2f}, min_keep={min_keep}, top_k={top_k})")
        except Exception:
            pass
    ...
    # 6) 合并保底分，并按 req_ids 顺序回包
    if floor_scores:
        scores_map = { **floor_scores, **{int(k): float(v) for k, v in scores_map.items()} }
    scores_list = [ float(scores_map.get(cid, 0.0)) for cid in req_ids ]

    # 7) 写 cache + 返回
    state.scores_cache[cache_key] = list(scores_list)
    res = {"tag": "scores_res", "scores": scores_list}
    if component is not None:   res["component"] = component
    if component_id is not None: res["component_id"] = component_id
    if getattr(args, 'echo_clause_ids', False): res["clause_ids"] = req_ids
...
def run_ea_server(host: str, port: int, args) -> None:
    ...
    _set_in_log_filter(getattr(args, 'mute_in', False),
                   getattr(args, 'mute_in_tags', ''))
    import socket, threading, json, sys, os, time
    # 置于文件顶部或 _ea_print_in 之前
    # 检查是否启用数据收集
    enable_data_collection = getattr(args, 'collect_proof_data', False)
    ...
    # 初始化状态（包含数据收集选项）
    output_dir = Path(run_dir) if enable_data_collection else None
    state = _EAState(enable_data_collection=enable_data_collection, output_dir=output_dir)
    
    # 如果启用数据收集，初始化数据收集器
    if enable_data_collection:
        problem_id = getattr(args, 'problem_id', None)
        if problem_id:
            state.init_data_collector(problem_id)
        else:
            # 如果没有指定problem_id，从时间戳生成一个
            import time
            problem_id = f"problem_{int(time.time())}"
            state.init_data_collector(problem_id)
            print(f"[EA] 使用自动生成的问题ID: {problem_id}")
        ...
        with conn:
            ...
            for msg in _ea_iter_json_messages(conn):
                ...
                if tag == "register_clauses":
                    ...
                    # 数据收集钩子
                    if state.enable_data_collection and state.data_collector:
                        state.data_collector.on_register_clauses(msg)
                elif tag == "scores_req":
                ...
                # Detect finish/timeout from iProver and request shutdown
                    ...
                    # 数据收集钩子
                    if state.enable_data_collection and state.data_collector:
                        if tag == "szs_result_out":
                            state.data_collector.on_szs_result_out(msg)
                        elif tag == "proof_out":
                            state.data_collector.on_proof_out(msg)
                            # 在收到proof_out后保存数据集
                            if state.current_problem_id:
                                problem_file = f"{state.current_problem_id}.p"  # 假设的文件名
                                state.save_collected_data(problem_file)
                ...
                # passive_clauses / given_clause / simplified_clauses 等可选消息，此处无需回包
                    ...
                    # 数据收集钩子 - 处理其他重要消息
                    if state.enable_data_collection and state.data_collector:
                        if tag == "passive_clauses":
                            state.data_collector.on_passive_clauses(msg)
                        elif tag == "given_clause":
                            state.data_collector.on_given_clause(msg)
                        elif tag == "simplified_clauses":
                            state.data_collector.on_simplified_clauses(msg)
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    p_srv.add_argument('--ranker-script', '--ranker', dest='ranker_script', type=str,
                       default=os.path.join(os.path.dirname(__file__), 'batch_ranker.py'),
                       help='Path to batch_ranker.py (alias: --ranker)')
    ...
    p_srv.add_argument('--prefilter-top-k', type=int, default=512,
                   help='Upper cap: after ratio/min_keep, keep at most top-K to send to LLM.')
    p_srv.add_argument('--prefilter-drop', type=float, default=0.0,
                    help='Drop ratio in [0,1). e.g. 0.7 means keep 30%% (ratio-first prefilter).')
    p_srv.add_argument('--prefilter-min-keep', type=int, default=0,
                    help='Minimum keep threshold. If N <= this, skip prefilter entirely.')
    p_srv.add_argument('--no-prefilter', dest='prefilter_enabled', action='store_false', default=True,
                    help='Disable EA-side prefiltering.')
    
    # 数据收集相关参数
    p_srv.add_argument('--collect-proof-data', action='store_true',
                       help='启用proof guidance数据收集模式，收集正负例训练数据')
    p_srv.add_argument('--dataset-output-format', type=str, choices=['json', 'jsonl'], default='jsonl',
                       help='数据集输出格式 (默认: jsonl)')
    p_srv.add_argument('--problem-id', type=str, default=None,
                       help='当前处理的问题ID，用于数据收集')
    
    p_srv.add_argument('--mute-in', action='store_true', default=False,
                   help='Do not print any [EA IN] lines to stdout')
    p_srv.add_argument('--mute-in-tags', type=str,
                   default='given_clause,passive_clauses,simplified_clauses',
                   help='Comma-separated EA IN tags to suppress from stdout')
